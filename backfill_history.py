...
Run once after seeding: python -m backfill_history
"""
import os
import re
...
def make_pc_url(card: Card) -> str | None:
    ...
    return f"https://www.pricecharting.com/game/{set_slug}/{name_slug}-{number}"
    ...
    with Session(engine) as session:
        ...
        with sync_playwright() as p:
            ...
            for card in cards:
                ...
                # Remove existing pricecharting snapshots for this card to avoid dupes
                existing_ts = {
                ...
                for dt, price_usd in points:
                    ...
                    snap = PriceSnapshot(
                        ...
                        pricecharting_price_hkd=Decimal(str(round(price_usd * USD_TO_HKD, 2))),
                        usd_to_hkd_rate=Decimal(str(USD_TO_HKD)),
                        scraped_at=dt,
                    )
                    session.add(snap)
                    inserted += 1
